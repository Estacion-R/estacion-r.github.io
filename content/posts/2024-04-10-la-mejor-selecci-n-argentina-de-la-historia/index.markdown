...
fechas <- seq.Date(from = as.Date("1873-01-01"),to=Sys.Date(),by="month")


tabla_ratings <- list() # Lista para guardar las tablas según fecha

tic() # Arranca el conteo del tiempo que tomará el proceso
...
row_odd <- seq_len(nrow(rating)) %% 2  # Selecciono las filas que son impares
  ...
  tabla_ratings[[i]] <- data.frame(pais = rating[row_odd==1,],elo =  rating[row_odd==0,],
                                   fecha = fechas[i])
  print(fechas[i])
...
tabla <- do.call(bind_rows, tabla_ratings) # Unifico todas las tablas en 1

# Guardo en formato .rds:
write_rds(tabla,glue::glue("data/elo_ranks_{fechas[1]}_{fechas[length(fechas)]}.rds"))
```
...
elo <- read_rds("elo_ranks_1873-01-01_2024-04-01.rds") %>% 
  mutate(elo = as.numeric(elo), # Convertimos a formato numérico el puntaje
         pais = ifelse(pais == "Falkland Islands","Islas Malvinas",pais)) # Corregimos nombre de las Islas Malvinas
...
elo %>% filter(fecha == max(fecha)) %>% # Seleccionamos los registros de la última fecha
  ...
  datatable(caption = glue::glue("Ranking Elo al {format(max(elo$fecha),'%d de %B de %Y')}")) # Tabla y título
```
...
<figure>
<img src="carasucias.webp" style="width:50.0%" alt="Selección 1957" />
...
<figure>
<img src="argentina_campeon_22.webp" style="width:50.0%" alt="Selección campeona Mundial 2022" />
...
elo_arg_germany <- elo %>% filter(pais %in% c("Argentina","Germany","West Germany")) %>% # Seleccionamos Alemania y Alemania Occidental para tomar la época de la división
  pivot_wider(names_from=pais,values_from=elo) %>% # Pivoteamos la tabla a formato wide
  mutate(Germany = ifelse(is.na(Germany),`West Germany`,Germany)) %>% # Asignamos el valor de Alemania Occidental cuando no hay dato en Alemania
   mutate(dif = Argentina - Germany) %>% # Diferencia de puntajes
  pivot_longer(cols=c("Argentina","Germany","dif"),names_to="var",values_to="val") %>% # Pivoteamos a formato long
  mutate(var_tipo = ifelse(var == "dif","dif","elo"), 
         var_tipo = factor(var_tipo,levels = c("elo","dif"),labels=c("elo","dif"))) # Convertimos a formato factor
```
...
ggplot(elo_arg_germany,aes(x=fecha,y=val)) + 
  geom_line(aes(color=var),data=elo_arg_germany %>% filter(var_tipo == "elo"))+ # Líneas de ambas series
  geom_col(data=elo_arg_germany %>% filter(var_tipo == "dif"),fill="gold")+ # Columnas con la diferencia entre ambas
  facet_grid(facets="var_tipo",scales = "free_y")+
  scale_color_manual(values=c("#75aadb","#DD0000"),name="")+
  theme(
  ...
  theme_minimal()+
  labs(x="",y="",title="Puntaje Elo de Argentina y Alemania y diferencia entre ambos",
